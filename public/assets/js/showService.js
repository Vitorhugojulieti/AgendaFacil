...
async function getData(){
    ...
    try {
        const response = await fetch(`http://localhost:8889/admin/service/getDataForDashboard?idService=${id}`);
        if (!response.ok) {
...
getData().then(data => {
    ...
    if(data != 0){
        ...
        containerChart.innerHTML =`<div class="w-full  text-grayInput flex flex-col gap-2 items-center justify-center p-12" >
                        ...
                        <span class="font-Urbanist font-semibold text-xl">O serviço não tem agendamentos realizados!</span>
                    </div>`;
    }
})
.catch(error => {
