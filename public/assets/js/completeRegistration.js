...
const previewLogo = new previewImage('#previewLogo','#inputLogo','#iconLogo','#spanLogo','#msgInputLogo',true);
previewLogo.init();

const previewImageCompany = new previewImage('#previewImage','#inputImage','#iconImage','#spanImage','#msgInputImage',true);
previewImageCompany.init();

//validation complete registration
const form = document.querySelector('#formCompleteRegistration');
form.addEventListener('submit',(e)=>{
    const hours = document.querySelectorAll('.hour');
    e.preventDefault();
    if(previewLogo.validateNotEmpty() == false && previewImageCompany.validateNotEmpty() == false){
        let error = `<span class="notification error" >Todas imagens são obrigatorias!</span>`;
        document.querySelector('main').insertAdjacentHTML('beforeend', error);
    }else if(hours.length == 0){
        let error = `<span class="notification error" >Nenhum horario adicionado!</span>`;
        document.querySelector('main').insertAdjacentHTML('beforeend', error);
    }else{
        e.target.submit();
    }
